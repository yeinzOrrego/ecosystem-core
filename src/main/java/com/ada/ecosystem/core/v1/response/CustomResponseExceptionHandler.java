...
public class CustomResponseExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    public ResponseEntity<Object> handleEcosystemExceptionException(final EcosystemException ex, final WebRequest request) {        
        logger.error("EcosystemException {}", ex);
        logger.info("EcosystemException WebRequest" + request.toString());
        if(ex.getLogProcess()  != null && !ex.getLogProcess().isEmpty()) {
        	logger.info("EcosystemException Log Process " + ex.getLogProcess());
        }
        final EcosystemError ecosystemError = new EcosystemError(HttpStatus.INTERNAL_SERVER_ERROR, 
        		ex.getLocalizedMessage(), 
        		ex.getErrors(),
